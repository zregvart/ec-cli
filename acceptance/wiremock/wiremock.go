...
func StartWiremock(ctx context.Context) (context.Context, error) {
	...
	// cwd is the directory where the test is run from i.e. $GITROOT/acceptance
	cwd, err := os.Getwd()
	if err != nil {
		return ctx, err
	}
	...
	req := testenv.TestContainersRequest(ctx, testcontainers.ContainerRequest{
		...
		Binds:        []string{fmt.Sprintf("%s:/recordings:z", path.Join(cwd, "wiremock", "recordings"))}, // relative to the running test, i.e. $GITROOT/acceptance
		Cmd: []string{
			"--root-dir=/recordings",
			"--verbose",
		},
